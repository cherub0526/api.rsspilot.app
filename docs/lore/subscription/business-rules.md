...
2026-09-22 盤點後的狀態（每位使用者每月）：

| 方案 | 典型 | 重度 | 極端 | 成本÷售價（重度） |
|---|---|---|---|---|
| Free | $1.49 | $1.65 | $1.69 | — |
| Pro | $4.70 | $6.18 | $7.56 | 0.48x |
| Advance | $15.52 | $45.77 | $59.09 | 1.83x |
...
與沒有上限的歷史重送，把它推到 1.96x / 4.27x。

**Advance 目前仍在判準之外（1.83x / 2.36x），而且是明知的。** 技術面的兩項調整
（歷史視窗、搜尋改走 parallel turbo）留著，但額度在降到 30 題之後依產品決定還原
成 50（`2026_09_22_120000_restore_advance_chat_limit`）。要在保留 50 題的前提下
回到判準內，得同時動三個旋鈕：推理力度降回 low、`max_tool_calls` 壓到 1、歷史
視窗從 10 輪收到 6 輪，合起來是 1.30x / 1.44x。沒有任何單一旋鈕做得到。

所以這條規則現在的狀態是「知道自己在哪裡，選擇先不收」，不是「忘了算」。真正該
先補的是**使用分布**——毛利對它極度敏感（典型 33%、重度 −97%），而目前沒有人在
量貼著上限用的人佔多少。
