...
系統 SHALL 支援以 Stripe Billing 建立訂閱。呼叫 `POST /v1/subscriptions`（`paymentMethod=stripe` 或預設）時，後端 SHALL 建立/取得 Stripe Customer、建立狀態為 `default_incomplete` 的 Stripe Subscription，並回傳 `publishable_key` 與 `client_secret`。

#### Scenario: New user creates Stripe subscription
...
- **THEN** 系統建立 Stripe Subscription（`default_incomplete`）並在 `stripes` 表記錄
